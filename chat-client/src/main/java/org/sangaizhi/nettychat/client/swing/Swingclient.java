package org.sangaizhi.nettychat.client.swing;
...
import org.sangaizhi.nettychat.client.Client;
import org.sangaizhi.nettychat.client.swing.constant.ButtonCommand;
	...
	public void actionPerformed(ActionEvent event) {
		...
		case ButtonCommand.LOGIN:
			try {
				...
				loginRequest.setUserName(playerName.getText());
				loginRequest.setPassword(passward.getText());
				
				//构建请求
				Request request = Request.valueOf(ModuleId.USER, UserCommand.LOGIN, loginRequest.getBytes());
		...
		case ButtonCommand.REGISTER:
			try {
				...
				registerRequest.setUserName(playerName.getText());
				registerRequest.setPassword(passward.getText());
				
				//构建请求
				Request request = Request.valueOf(ModuleId.USER, UserCommand.REGISTER_AND_LOGIN, registerRequest.getBytes());
				client.sendRequest(request);
			} catch (Exception e) {
				tips.setText("无法连接服务器");
			}
		...
		case ButtonCommand.SEND:
			try {
				...
				if(StringUtils.isEmpty(targetPlayer.getText()) && !StringUtils.isEmpty(message.getText())){
					...
					//构建请求
					Request request = Request.valueOf(ModuleId.CHAT, ChatCommand.PUBLIC_CHAT, publicChatRequest.getBytes());
					client.sendRequest(request);
				}else{
					...
					privateChatRequest.setTargetUserId(palyerId);
					
					//构建请求
					Request request = Request.valueOf(ModuleId.CHAT, ChatCommand.PRIVATE_CHAT, privateChatRequest.getBytes());
					client.sendRequest(request);
				}
			} catch (Exception e) {
				tips.setText("无法连接服务器");
