package org.sangaizhi.nettychat.client;

import org.sangaizhi.nettychat.client.scanner.Invoker;
import org.sangaizhi.nettychat.client.scanner.InvokerHoler;
...
public class ClientHandler extends SimpleChannelInboundHandler<Response> {
	...
	private void handlerResponse(Response response){
		...
		//获取命令执行器
		Invoker invoker = InvokerHoler.getInvoker(response.getModule(), response.getCommand());
		if(invoker != null){
			try {
			...
			//找不到执行器
			System.out.println(String.format("module:%s  cmd:%s 找不到命令执行器", response.getModule(), response.getCommand()));
		}
	}
